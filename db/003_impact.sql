...
WITH RECURSIVE impact AS (
  SELECT
    ...
    1 AS impact_depth,
    ARRAY[p_file_id, r.target_file_id]::UUID[] AS visited,
  ...
  SELECT
    ...
    LEAST(i.min_confidence, r.confidence)::NUMERIC(5,4),
...
SELECT DISTINCT ON (affected_file_id)
  ...
  i.impact_depth,
  array_to_string(i.rel_path, ' -> ') AS path,
  i.min_confidence,
  i.relationship_count
FROM impact i
JOIN files f ON f.id = i.affected_file_id
ORDER BY affected_file_id, min_confidence DESC, impact_depth ASC;
$$;

COMMENT ON FUNCTION analyze_impact(UUID) IS
'Walks typed downstream relationships recursively and returns the strongest confidence path to each affected file.';
